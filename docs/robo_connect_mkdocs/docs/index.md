...
It is a generic library providing robot functionality to TwinCAT (TC) users. It simplifies integration of any robot into the TC system by providing a common set of functionalities to PLC developers. It is designed to be familiar to users of Beckhoff’s Motion library, TC_MC2, but can also be used with Object Oriented Programming (OOP) techniques.
...
There is no requirement for users of RoboConnect to make their libraries open source or freely available. However, if users are making a new connectivity library for a particular robot brand & model, it will help the community grow to make this open source, either through the RoboConnect organisation's [GitHub](https://github.com/plc-roboconnect), or through their own public repo.

##	One Abstraction, Many Robots
RoboConnect is an abstraction layer. It does not offer any out-of-the box kinematics. It is designed to be an abstraction around robot controllers, such that one robot controller can be switched for another, with no change to the PLC code required to control it (if the degrees of freedom, reach and load capabilities match). RoboConnect can be used with robots of any given form, from two to 'n' axes.
...
<figure>
  <img src="images/roboconnect-dobot-connection.png"
       alt="robo connect to dobot">
  <figcaption>RoboConnect abstracting a common 6 axis robot - robot controller handles kinematics</figcaption>
</figure>
...
<figure>
  <img src="images/roboconnect-multi-robots.png"
       alt="one application, any robot">
  <figcaption>High reuse of application code with any robot</figcaption>
</figure>
